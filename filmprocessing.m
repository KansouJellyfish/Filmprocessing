...
Rb1=im2double(imread(fullfile(filepath,'black1.tiff'))); % 黑点（胶卷）
Rw1=im2double(imread(fullfile(filepath,'white1.tiff'))); % 白点（胶卷）
R0=im2double(imread(fullfile(filepath,'flat.tiff'))); % H=0
Rbavg=squeeze(mean(Rb,[1 2])); % 求平均，沿第一维和第二维计算均值，去掉大小为1的维度，使结果变成1*3
Rwavg=squeeze(mean(Rw,[1 2]));
Rb1=squeeze(mean(Rb1,[1 2]));
Rw1=squeeze(mean(Rw1,[1 2]));
...
for n=1111:2222
    ...
    for i=1:3
        ...
        % 根据比尔-朗博定律，将通光率转换为吸收率A
        A(:,:,i)=-log10(T(:,:,i));

        % logH=-1.5代表18%灰卡在正常曝光下的值，令k=该点对应的密度D0/吸收率A0，使吸收率与密度可以换算
        T0(i)=(R0avg(i)-Rbavg(i))/(Rwavg(i)-Rbavg(i)); % 计算标准曝光的通光率
        A0(i)=-log10(T0(i));
        ...
        x = x(validIdx);
        y = y(validIdx);
        D0(i)=interp1(x,y,-1.5,'linear'); % 插值得到正常曝光时的密度D值
        k(i)=D0(i)/A0(i);
        ...
        % 计算胶卷的黑点、白点对应D
        Tb1(i)=(Rb1(i)-Rbavg(i))/(Rwavg(i)-Rbavg(i));
        Ab1(i)=-log10(Tb1(i));
        Db1(i)=Ab1(i)*k(i);
        Hb1(i)=interp1(y,x,Db1(i),'linear');
        Tw1(i)=(Rw1(i)-Rbavg(i))/(Rwavg(i)-Rbavg(i));
        Aw1(i)=-log10(Tw1(i));
        Dw1(i)=Aw1(i)*k(i);
        Hw1(i)=interp1(y,x,Dw1(i),'linear');
    end
